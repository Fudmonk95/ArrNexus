(function(){
  ...
  async function pollJob(id,node){try{const r=await fetch('/api/jobs/'+id,{headers:{Accept:'application/json'}});if(!r.ok)return;const d=await r.json();const j=d.job||{};const done=(j.completed||0)+(j.reviewed||0)+(j.rejected||0)+(j.failed||0),total=j.total||1,p=Math.round(done/total*100);node.innerHTML='<strong>Import job #'+id+' · '+(j.status||'running').replaceAll('_',' ')+'</strong><small>'+(j.completed||0)+' complete · '+(j.reviewed||0)+' manual review · '+(j.rejected||0)+' rejected · '+(j.failed||0)+' failed · '+(j.message||'')+'</small><div class="job-progress"><i style="width:'+p+'%"></i></div>';if(['queued','running'].includes(j.status))setTimeout(()=>pollJob(id,node),1500);}catch(_e){}}
...
/* ArrNexus v10 — native release discovery, install progress and automatic reload. */
(function(){
  ...
  function showModal(meta,force=false){const root=modal();if(!root||!meta?.update_available)return;const dismissed=localStorage.getItem('arrnexus:update-dismissed');if(!force&&dismissed===String(meta.latest||''))return;root.hidden=false;root.setAttribute('aria-hidden','false');const target=root.querySelector('[data-update-target]');if(target)target.textContent='v'+(meta.latest||'');const msg=root.querySelector('#v10UpdateMessage');if(msg){msg.textContent=meta.installable?'ArrNexus will verify the release, back up the database, run the full validator and restart automatically.':(meta.reason||'The release is visible but cannot be installed automatically from this runtime.');}const install=root.querySelector('[data-install-update]');if(install)install.disabled=!meta.installable;}
  function hideModal(){const root=modal();if(!root)return;root.hidden=true;root.setAttribute('aria-hidden','true');if(latestMeta?.latest)localStorage.setItem('arrnexus:update-dismissed',String(latestMeta.latest));}
  function decorate(meta){latestMeta=meta;const badge=document.querySelector('.nx-version-badge');if(meta?.update_available&&badge){badge.classList.add('has-update');const em=badge.querySelector('[data-version-update]');if(em){em.hidden=false;em.textContent='UPDATE '+(meta.latest||'');}}if(meta?.error)setResult('<span class="state-error">Update check failed: '+escapeHtml(meta.error)+'</span>');else if(meta?.update_available)setResult('Update available: <strong>v'+escapeHtml(meta.latest)+'</strong> · '+(meta.installable?'ready for in-app installation':'manual update required'));else setResult('ArrNexus is up to date · <strong>v'+escapeHtml(meta?.current||'')+'</strong>');}
  async function check(forceModal=false){try{const r=await fetch('/api/update-check',{headers:{Accept:'application/json','Cache-Control':'no-cache'},credentials:'same-origin',cache:'no-store'});if(!(r.headers.get('content-type')||'').includes('json'))return null;const d=await r.json();decorate(d);localStorage.setItem('arrnexus:update-status',JSON.stringify({at:Date.now(),data:d}));if(d.update_available)showModal(d,forceModal);return d;}catch(err){setResult('<span class="state-error">Update check failed: '+escapeHtml(err.message)+'</span>');return null;}}
  async function pollStatus(){if(statusTimer)clearTimeout(statusTimer);try{const r=await fetch('/api/update-status?ts='+Date.now(),{headers:{Accept:'application/json'},credentials:'same-origin',cache:'no-store'});if(r.ok){const d=await r.json();const root=modal(),msg=root?.querySelector('[data-update-status]'),bar=root?.querySelector('[data-update-progress]'),fill=bar?.querySelector('i');if(msg)msg.textContent=d.message||d.state||'';if(bar){bar.hidden=false;if(fill)fill.style.width=Math.max(3,Math.min(100,Number(d.progress||0)))+'%';}setResult(escapeHtml(d.message||d.state||'Updating…'));if(d.state==='failed'||d.state==='rolled_back'){installing=false;const b=root?.querySelector('[data-install-update]');if(b){b.disabled=false;b.textContent='Try again';}return;}if(d.state==='running'&&d.active&&latestMeta?.latest&&String(d.active).replace(/^v/,'')===String(latestMeta.latest).replace(/^v/,'')){location.reload();return;}}}catch(_e){/* restart window: health polling below will recover */}
    if(installing){try{const h=await fetch('/api/health?ts='+Date.now(),{cache:'no-store'});if(h.ok){const j=await h.json();if(latestMeta?.latest&&String(j.version||'').replace(/^v/,'')===String(latestMeta.latest).replace(/^v/,'')){location.reload();return;}}}catch(_e){}statusTimer=setTimeout(pollStatus,1600);}}
  async function install(){if(installing)return;if(!latestMeta?.update_available){latestMeta=await check(true);if(!latestMeta?.update_available)return;}if(!latestMeta.installable){showModal(latestMeta,true);return;}if(!window.confirm('Install ArrNexus v'+latestMeta.latest+' now? ArrNexus will back up the database, validate the release and restart itself.'))return;installing=true;localStorage.removeItem('arrnexus:update-dismissed');showModal(latestMeta,true);const root=modal(),button=root?.querySelector('[data-install-update]');if(button){button.disabled=true;button.textContent='Installing…';}try{const r=await fetch('/api/update-install',{method:'POST',headers:{Accept:'application/json'},credentials:'same-origin'});const d=await r.json();if(!r.ok)throw new Error(d.detail||'Update could not start');const msg=root?.querySelector('[data-update-status]');if(msg)msg.textContent=d.message||'Update started';pollStatus();}catch(err){installing=false;if(button){button.disabled=false;button.textContent='Install update';}const msg=root?.querySelector('[data-update-status]');if(msg)msg.textContent=err.message;}}
  document.addEventListener('click',e=>{if(e.target.closest('[data-update-dismiss]')){hideModal();return;}if(e.target.closest('[data-install-update]')){e.preventDefault();install();return;}if(e.target.closest('[data-check-update]')){window.setTimeout(()=>check(true),0);}});
