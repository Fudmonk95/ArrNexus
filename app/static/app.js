(function(){
  ...
  async function pollJob(id,node){try{const r=await fetch('/api/jobs/'+id,{headers:{Accept:'application/json'}});if(!r.ok)return;const d=await r.json();const j=d.job||{};const done=(j.completed||0)+(j.rejected||0)+(j.failed||0),total=j.total||1,p=Math.round(done/total*100);node.innerHTML='<strong>Import job #'+id+' · '+(j.status||'running').replaceAll('_',' ')+'</strong><small>'+(j.completed||0)+' complete · '+(j.rejected||0)+' rejected · '+(j.failed||0)+' failed · '+(j.message||'')+'</small><div class="job-progress"><i style="width:'+p+'%"></i></div>';if(['queued','running'].includes(j.status))setTimeout(()=>pollJob(id,node),1500);}catch(_e){}}
