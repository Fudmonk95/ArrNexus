...
class DecypharrClient:
    ...
    @property
    def headers(self): return {'Authorization':f'Bearer {self.token}','User-Agent':'ArrNexus/6.0'}
    ...
    async def version(self):
        ...
        if not self.url: raise DecypharrError('Decypharr URL is not configured')
        async with httpx.AsyncClient(timeout=8.0,follow_redirects=True,headers={'User-Agent':'ArrNexus/6.0'}) as c: r=await c.get(self.url+'/version')
